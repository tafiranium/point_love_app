...
class SaleManager {
    ...
    async fetchReceiptHtml(url) {
        ...
        // главная таблица
        let detailsTable = html.querySelector("table.detail-view tbody")

        // проверка на возврат
        let is_returned = (!detailsTable.querySelector("tr:nth-child(17)").classList.contains('cssDisplayNone'))  

        // Получаем селекторы из конфига
        // details: {traffic: [19, "td", true], reason: [21, "td", true], comment: [22, "td", true]}
        const cfg = configure || await getDefaultConfig();
        const det = cfg.details || {};
        
        // Формируем селекторы для трафика, причины и комментария
        // Формат: [номер_строки, селектор, innerText]
        const trafficRow = det.traffic ? det.traffic[0] : 19;
        const trafficSel = det.traffic ? det.traffic[1] : 'td';
        const reasonRow = det.reason ? det.reason[0] : 21;
        const reasonSel = det.reason ? det.reason[1] : 'td';
        const commentRow = det.comment ? det.comment[0] : 22;
        const commentSel = det.comment ? det.comment[1] : 'td';
        
        // Получаем данные используя селекторы из конфига
        const trafficEl = detailsTable.querySelector(`tr:nth-child(${trafficRow}) ${trafficSel}`);
        const reasonEl = detailsTable.querySelector(`tr:nth-child(${reasonRow}) ${reasonSel}`);
        const commentEl = detailsTable.querySelector(`tr:nth-child(${commentRow}) ${commentSel}`);
        
        let details = [
            trafficEl ? trafficEl.innerText : '', // трафик
            reasonEl ? reasonEl.innerText : '',   // причина непокупки
            commentEl ? commentEl.innerText : ''  // комментарий
        ]
